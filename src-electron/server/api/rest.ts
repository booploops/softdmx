...
function isShowDocumentV1(payload: unknown): payload is ShowDocumentV1 {
  return !!payload && typeof payload === 'object' && isSupportedShowVersion((payload as { version?: string }).version);
}
...
export function registerRemoteRestRoutes(server: FastifyInstance, ctx: RemoteContext): void {
  const requiredToken = process.env.SOFTDMX_API_TOKEN?.trim();

  server.register((instance, _opts, done) => {
    if (requiredToken) {
      instance.addHook('onRequest', (request, reply, hookDone) => {
        const token = getRequestToken(request.headers as Record<string, unknown>);
    ...
    instance.post<{ Body: ShowDocumentV1 }>('/show', (request, reply) => {
      const body = request.body;
      if (!isShowDocumentV1(body)) {
    ...
    instance.post<{ Body: { enabled: boolean } | boolean }>('/audio/enabled', (request, reply) => {
      const enabled =
        typeof request.body === 'boolean'
          ? request.body
    ...
    instance.post<{ Body: { mapping: ShowAudioMapping } }>('/audio/mappings', (request, reply) => {
    ...
    instance.patch<{ Body: AudioMappingUpdatePayload }>('/audio/mappings/:id', (request, reply) => {
    ...
    instance.post<{ Body: { path: string; value: number; attributeType?: string } }>(
      '/channel/set',
      (request, reply) => {

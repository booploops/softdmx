...
import type { AttributeDefinition, FixtureModeDefinition } from 'src/types/attributes';
import type { FixtureChannelDefinition, FixtureDefinition, WidgetConfiguration } from 'src/types/fixture';
import {
...
import { inferMergeForFeature, mapGdtfFeatureToAttributeFeature, sanitizeFixtureId, sanitizeModeId } from './feature-map.ts';
...
function buildModeChannels(
  ...
  for (const dmxChannel of readGdtfChannels(mode)) {
    ...
    const offset = typeof offsetRaw === 'string' ? Number(offsetRaw.split(',')[0]) : channels.length + 1;
    const attributeName = resolveChannelAttributeName(dmxChannel);
    const attribute = attributeMap.get(attributeName);
    const featureRaw = attribute?.feature ?? mapGdtfFeatureToAttributeFeature(undefined, attributeName);
...
export function gdtfParseResultToFixture(result: GdtfParseResult, preferredModeIndex = 0): FixtureDefinition {
  const fixtureType = getFixtureTypeFromRoot(result.root);
  const fixtureName = typeof fixtureType['@_Name'] === 'string' ? fixtureType['@_Name'] : 'Imported Fixture';
  const fixtureTypeId =
...
export function loadFixtureFromGdtf(bytes: Uint8Array, fileName?: string, preferredModeIndex = 0): FixtureDefinition {

...
import type { ShowBindings } from './bindings';
import type { Cue } from './cue';
import type { ShowDeskConfig, ShowTouchConfig } from './desk';
...
export interface ShowfileFixture {
  name: string;
  fixtureId: string;
  modeId?: string;
...
export interface ShowfileGroup {
  name: string;
  fixtures: string[];
  color?: string;
}
...
export function createEmptyShow(name = 'Untitled Show'): ShowDocumentV1 {
  const now = new Date().toISOString();
  const doc: ShowDocumentV1 = {
    version: '1.5',

...
export function useRemoteAPI() {
  ...
  let pendingRemoteShow: ShowDocumentV1 | null = null;
  ...
  function applySimpleRemoteMerge(remoteShow: ShowDocumentV1) {
  ...
  function promptRemoteConflict(remoteShow: ShowDocumentV1) {
  ...
  function handleRemoteEvents() {
    ...
    socket.on('show:state', (show) => {
      ...
      const incomingShow = show as ShowDocumentV1;
  ...
  onMounted(handleRemoteEvents);

  onBeforeUnmount(() => {
    dismissShowConflictNotice();
    socket.off('remote:scratch:clear');
    socket.off('remote:scratch:set');
    socket.off('remote:preset:fire');
    socket.off('remote:cue:play');
    socket.off('remote:cue:stop');
    socket.off('remote:cue:stack:go');
    socket.off('remote:blackout');
    socket.off('remote:grandmaster');
    socket.off('remote:playbackbus');
    socket.off('remote:effect:set');
    socket.off('remote:audio:setEnabled');
    socket.off('remote:audio:mappings:create');
    socket.off('remote:audio:mappings:update');
    socket.off('remote:audio:mappings:delete');
    socket.off('show:state');
  });

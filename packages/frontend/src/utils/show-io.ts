...
import { createEmptyShow } from '../types/show-document.ts';
import { migrateShowDocument } from './show-migrate.ts';
import { defaultGroupColorForIndex, normalizeGroupColor } from './group-colors.ts';
...
import { CURRENT_SHOW_VERSION, isSupportedShowVersion } from './show-version.ts';
import { ensureDefaultPresetPool } from './preset-pool.ts';
import {
  normalizeLatencyMs,
  normalizePhaseOffsetBeats,
  normalizeSignedOffsetMs,
} from './sync-compensation.ts';

export function validateShowDocument(data: unknown): ShowDocumentV1 {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid show document: not an object');
  }

  const doc = migrateShowDocument(data as Partial<ShowDocumentV1>);
  ...
  const partialDoc = doc as ShowDocumentV1;
  const desk = doc.desk ?? createDefaultDeskConfig();
  const touch = doc.touch ?? createDefaultTouchConfig({ ...partialDoc, version: normalizedVersion, desk });
  ...
  return {
    ...
    timecode: {
      ...
      fps:
        typeof doc.timecode?.fps === 'number' && Number.isFinite(doc.timecode.fps)
          ...
          : 30,
      source:
        doc.timecode?.source === 'ltc' || doc.timecode?.source === 'mtc' || doc.timecode?.source === 'osc'
          ? doc.timecode.source
          : 'osc',
      ...
      latencyMs:
        typeof doc.timecode?.latencyMs === 'number' && Number.isFinite(doc.timecode.latencyMs)
          ...
          : 0,
      globalOffsetMs:
        typeof doc.timecode?.globalOffsetMs === 'number' && Number.isFinite(doc.timecode.globalOffsetMs)
    ...
    oscSync: {
      mediaLatencyMs:
        typeof doc.oscSync?.mediaLatencyMs === 'number' && Number.isFinite(doc.oscSync.mediaLatencyMs)
...
function normalizeCue(cue: ShowDocumentV1['cues'][number]): ShowDocumentV1['cues'][number] {
  return {
    ...cue,
    ...
    stack: cue.stack ?? [],
    parts: cue.parts ?? [],
    created: typeof cue.created === 'string' ? cue.created : new Date(cue.created as unknown as string).toISOString(),
    modified: typeof cue.modified === 'string' ? cue.modified : new Date(cue.modified as unknown as string).toISOString(),
  };
}

export function parseShowDocument(yamlOrJson: string): ShowDocumentV1 {
  const parsed = YAML.parse(yamlOrJson);
  return validateShowDocument(parsed);
}

export function serializeShowDocument(doc: ShowDocumentV1): string {
  const exportDoc: ShowDocumentV1 = {
    ...doc,
...
export function downloadShowDocument(doc: ShowDocumentV1, filename?: string): boolean {
  try {
...
export async function loadShowDocumentFromFile(file: File): Promise<ShowDocumentV1> {

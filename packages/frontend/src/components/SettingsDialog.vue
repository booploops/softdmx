...
const outputOptions = [
  ...
  { label: 'GridNode Output', value: 'gridnode' },
  { label: 'DMX USB Pro', value: 'dmx_usb' },
];

const usbProtocolOptions = [
  { label: 'Enttec DMX USB Pro', value: 'enttec_pro' },
  { label: 'Open DMX USB', value: 'open_dmx' },
];

const timecodeLocked = computed(() => timecodeStore.locked);
...
<template>
  <VueFinalModal
    ...
    @closed="emit('cancel')"
  >
    <XDialogWindow style="width: min(840px, 96vw);">
      <XDialogTitlebar
        title="Output & Sync"
        @close="emit('cancel')"
      />

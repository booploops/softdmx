...
export function loadPluginsFromIds(pluginIds: string[]): void {
  ...
  for (const pluginId of new Set(pluginIds)) {
    ...
    if (!manifestPath) {
      let fixtureFromLegacyDir: FixtureDefinition | undefined;
      try {
        fixtureFromLegacyDir = loadFixtureYamlFromPath(`../fixtures/${pluginId}.yaml`);
      } catch {
        try {
          fixtureFromLegacyDir = loadFixtureYamlFromPath(`../fixtures/${pluginId}.yml`);
        } catch {
